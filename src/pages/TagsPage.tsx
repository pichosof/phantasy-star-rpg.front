...
const ENTITY_DETAIL_FIELDS: Record<string, Array<{ label: string; key: string }>> = {
  beasts:   [{ label: 'Type', key: 'type' }, { label: 'Habitat', key: 'habitat' }, { label: 'Description', key: 'description' }],
  npcs:     [{ label: 'Role', key: 'role' }, { label: 'Location', key: 'location' }, { label: 'Description', key: 'description' }],
  cities:   [{ label: 'Description', key: 'description' }],
  dungeons: [{ label: 'Type', key: 'type' }, { label: 'Region', key: 'region' }, { label: 'Description', key: 'description' }],
  worlds:   [{ label: 'Description', key: 'description' }],
  players:  [{ label: 'Level', key: 'level' }, { label: 'Background', key: 'background' }],
  lores:    [{ label: 'Category', key: 'category' }, { label: 'Content', key: 'content' }],
  quests:   [{ label: 'Status', key: 'status' }, { label: 'Description', key: 'description' }, { label: 'Reward', key: 'reward' }],
};
...
const EntityDetailModal: React.FC<{
  ...
  return (
    <Modal visible onCancel={onClose} footer={null} title={
      <Space size={8}>
      ...
      <Space direction="vertical" size={12} style={{ width: '100%' }}>
        ...
        {fields.map(({ label: fl, key }) => {
          ...
          return (
            <div key={key}>
              <Typography.Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 2 }}>{fl}</Typography.Text>
...
const EntityGroup: React.FC<{ label: string; entityType: string; items: any[]; onSelect: (type: string, item: any) => void }> = ({ label, entityType, items, onSelect }) => {
  if (items.length === 0) return null;
  return (
    <div>
      <Typography.Text type="secondary" style={{ fontSize: 12, fontWeight: 600 }}>{label}</Typography.Text>
...
export const TagsPage: React.FC = () => {
  ...
  async function saveEdit(tag: TagType) {
    try {
      ...
      if (selectedTag?.id === tag.id) setSelectedTag((prev) => prev ? { ...prev, name: editName.trim(), color: editColor } : prev);
      setEditingId(null);
  ...
  async function handleDelete(id: number) {
    try {
      ...
      if (selectedTag?.id === id) { setSelectedTag(null); setTagEntities(null); }
  ...
  const totalEntities = tagEntities
    ? Object.values(tagEntities).filter(Array.isArray).reduce((s: number, arr: any[]) => s + arr.length, 0)
    : 0;

  return (
    <>
      <PageTitle>Tags</PageTitle>
      <EntityDetailModal entityType={detailType} item={detailItem} onClose={() => { setDetailType(null); setDetailItem(null); }} />

      <div style={{ display: 'grid', gridTemplateColumns: mobileOnly ? '1fr' : 'minmax(280px,360px) 1fr', gap: 16, alignItems: 'start' }}>
        {/* ── Left: tag list ──────────────────────────────────────────────── */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Card density="dense">
            <Space direction="vertical" size={10} style={{ width: '100%' }}>
              <Typography.Title level={4} style={{ margin: 0 }}>🏷️ Tags</Typography.Title>
              ...
              <form onSubmit={(e) => void handleCreate(e)}>
                <Space direction="vertical" size={8} style={{ width: '100%' }}>
                  ...
                  <Space size={8}>
                    ...
                    <input type="color" value={newColor} onChange={(e) => setNewColor(e.target.value)} style={{ width: 36, height: 28, padding: 2, border: '1px solid #d9d9d9', borderRadius: 4, cursor: 'pointer' }} />
              ...
              <Space direction="vertical" size={4} style={{ width: '100%' }}>
                {filtered.map((tag) => (
                  ...
                  >
                    {editingId === tag.id ? (
                      <>
                        <input type="color" value={editColor} onChange={(e) => setEditColor(e.target.value)} style={{ width: 28, height: 22, padding: 1, border: '1px solid #d9d9d9', borderRadius: 4, cursor: 'pointer' }} />
                        <input
                          style={{ flex: 1, padding: '2px 6px', border: '1px solid #d9d9d9', borderRadius: 4, background: 'transparent', color: 'inherit', fontSize: 13 }}
                          value={editName}
                          onChange={(e) => setEditName(e.target.value)}
                          onKeyDown={(e) => { if (e.key === 'Enter') void saveEdit(tag); if (e.key === 'Escape') setEditingId(null); }}
                          autoFocus
                          onClick={(e) => e.stopPropagation()}
                        />
                        <Button size="small" type="primary" onClick={(e) => { e.stopPropagation(); void saveEdit(tag); }}>✓</Button>
                        <Button size="small" onClick={(e) => { e.stopPropagation(); setEditingId(null); }}>✕</Button>
                      </>
                    ) : (
                      <>
                        <Tag color={tag.color} icon={<TagOutlined />} style={{ margin: 0, flex: 1 }}>{tag.name}</Tag>
                        {isGM && (
              ...
              {entitiesLoading ? (
                ...
                <Space direction="vertical" size={16} style={{ width: '100%' }}>
                  {Object.entries(ENTITY_LABELS).map(([key, label]) => {
                    ...
                    return <EntityGroup key={key} label={label} entityType={key} items={items ?? []} onSelect={(type, item) => { setDetailType(type); setDetailItem(item); }} />;
                  })}
                </Space>
              )}
            </Space>
