    ...
    <Collapse defaultActiveKey={['identity','stats']} style={{ width: '100%' }}>
      ...
      <Collapse.Panel header="Identidade" key="identity">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: 10 }}>
          {([['description','Descrição'],['classLevel','Classe/Nível'],['race','Raça'],['theme','Tema'],['size','Tamanho'],['speedFt','Velocidade (ft)'],['gender','Gênero'],['homeWorld','Mundo Natal'],['alignment','Alinhamento'],['deity','Divindade'],['player','Jogador']] as const).map(([k,lbl]) => (
      ...
      {/* Ability Scores */}
      <Collapse.Panel header="Atributos" key="stats">
      ...
      <Collapse.Panel header="Combate" key="combat">
        <Space wrap size={8} style={{ marginBottom: 12 }}>
          <CalcBox label="Iniciativa" value={sign(calc.initiative)} color="#597ef7" />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: 8 }}>
          <LabelInput label="Bônus Armadura (EAC)"><InputNumber style={{ width:'100%' }} value={data.armorBonus ?? 0} onChange={(v) => set({ armorBonus: v ?? 0 })} /></LabelInput>
          <LabelInput label="Bônus Armadura (KAC)"><InputNumber style={{ width:'100%' }} value={data.armorKacBonus ?? 0} onChange={(v) => set({ armorKacBonus: v ?? 0 })} /></LabelInput>
          <LabelInput label="Max DEX (armadura)"><InputNumber style={{ width:'100%' }} value={data.armorMaxDex} placeholder="∞" onChange={(v) => set({ armorMaxDex: v ?? undefined })} /></LabelInput>
          ...
          <LabelInput label="Misc Iniciativa"><InputNumber style={{ width:'100%' }} value={data.initiativeMisc ?? 0} onChange={(v) => set({ initiativeMisc: v ?? 0 })} /></LabelInput>
          <LabelInput label="DR"><Input value={data.dr ?? ''} onChange={(e) => set({ dr: e.target.value })} /></LabelInput>
          <LabelInput label="Resistências"><Input value={data.resistances ?? ''} onChange={(e) => set({ resistances: e.target.value })} /></LabelInput>
        </div>
        <Divider style={{ margin: '12px 0 8px' }} />
        <Typography.Text strong style={{ fontSize: 12 }}>Saving Throws</Typography.Text>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginTop: 8 }}>
          {([['fort','Fortitude','con',calc.fort],['ref','Reflexo','dex',calc.ref],['will','Vontade','wis',calc.will]] as const).map(([k,lbl,attr,total]) => (
        ...
        <Divider style={{ margin: '12px 0 8px' }} />
        <Typography.Text strong style={{ fontSize: 12 }}>Bônus de Ataque</Typography.Text>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(130px,1fr))', gap: 8, marginTop: 8 }}>
          ...
          <CalcBox label={`Corpo a corpo (${sign(calc.mods.str)} STR)`} value={sign(calc.melee)} color="#ff4d4f" />
          <CalcBox label={`À distância (${sign(calc.mods.dex)} DEX)`} value={sign(calc.ranged)} color="#40a9ff" />
          <CalcBox label={`Arremesso (${sign(calc.mods.str)} STR)`} value={sign(calc.thrown)} color="#ffc53d" />
          <LabelInput label="Misc Corpo a corpo"><InputNumber style={{ width:'100%' }} value={data.meleeMisc ?? 0} onChange={(v) => set({ meleeMisc: v ?? 0 })} /></LabelInput>
          <LabelInput label="Misc À distância"><InputNumber style={{ width:'100%' }} value={data.rangedMisc ?? 0} onChange={(v) => set({ rangedMisc: v ?? 0 })} /></LabelInput>
          <LabelInput label="Misc Arremesso"><InputNumber style={{ width:'100%' }} value={data.thrownMisc ?? 0} onChange={(v) => set({ thrownMisc: v ?? 0 })} /></LabelInput>
        </div>
      </Collapse.Panel>

      {/* HP */}
      <Collapse.Panel header="Pontos de Vida" key="hp">
          ...
          {([['Stamina','staminaTotal','staminaCurrent','#40a9ff'],['Hit Points','hpTotal','hpCurrent','#73d13d'],['Resolve','resolveTotal','resolveCurrent','#9254de']] as const).map(([lbl,tk,ck,color]) => (
            <div key={tk} style={{ padding: 10, borderRadius: 8, background: `${color}10`, border: `1px solid ${color}33`, textAlign:'center' }}>
              ...
              <Space direction="vertical" size={4} style={{ width:'100%' }}>
                ...
                <InputNumber style={{ width:'100%' }} placeholder="Atual" value={(data as any)[ck]} onChange={(v) => set({ [ck]: v ?? undefined } as any)} />
              </Space>
            </div>
          ))}
        </div>
      ...
      {/* Skills */}
      <Collapse.Panel header="Perícias" key="skills">
        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '2px 0', marginBottom: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '60px 80px 60px 60px', gap: 4, padding: '0 4px', fontSize: 11, opacity: 0.6 }}>
            <span>Ranks</span><span>Classe (+3)</span><span>Misc</span><span>Total</span>
          </div>
        </div>
        {SF_SKILLS.map((sk) => {
          ...
          return (
            <div key={sk.key} style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', alignItems: 'center', gap: 4, marginBottom: 4 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '60px 80px 60px 60px', gap: 4, alignItems: 'center' }}>
                ...
                <Switch size="small" checked={!!s.classBonus} onChange={(v) => setSkill(sk.key, { classBonus: v })} checkedChildren="Classe" unCheckedChildren="—" />
        ...
        <Space size={12}>
          <LabelInput label="Ranks/Nível"><InputNumber value={data.skillRanksPerLevel} onChange={(v) => set({ skillRanksPerLevel: v ?? undefined })} style={{ width: 80 }} /></LabelInput>
          <LabelInput label="Penalidade de Armadura"><InputNumber value={data.armorCheckPenalty ?? 0} onChange={(v) => set({ armorCheckPenalty: v ?? 0 })} style={{ width: 80 }} /></LabelInput>
        </Space>
      </Collapse.Panel>

      {/* Weapons */}
      <Collapse.Panel header="Armas" key="weapons">
        {weapons.map((w, i) => (
          <div key={i} style={{ marginBottom: 12, padding: 10, borderRadius: 8, background: 'rgba(128,128,128,0.05)', border: '1px solid rgba(128,128,128,0.1)' }}>
            <Typography.Text strong style={{ fontSize: 12, opacity: 0.6 }}>Arma {i + 1}</Typography.Text>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(120px,1fr))', gap: 6, marginTop: 6 }}>
              {([['name','Nome'],['level','Nível'],['attackBonus','Ataque'],['damage','Dano'],['critical','Crítico'],['range','Alcance'],['type','Tipo'],['ammoUsage','Munição/Uso'],['special','Especial']] as const).map(([k,lbl]) => (
                <LabelInput key={k} label={lbl}>
      ...
      {/* Armor equipment */}
      <Collapse.Panel header="Armadura Equipada" key="armor">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 10 }}>
          <LabelInput label="Modelo"><Input value={data.armorModel ?? ''} onChange={(e) => set({ armorModel: e.target.value })} /></LabelInput>
          <LabelInput label="Nível"><InputNumber style={{ width:'100%' }} value={data.armorLevel} onChange={(v) => set({ armorLevel: v ?? undefined })} /></LabelInput>
          <LabelInput label="EAC Bônus"><InputNumber style={{ width:'100%' }} value={data.armorEacBonus ?? 0} onChange={(v) => set({ armorEacBonus: v ?? 0 })} /></LabelInput>
          <LabelInput label="KAC Bônus"><InputNumber style={{ width:'100%' }} value={data.armorKacBonusEq ?? 0} onChange={(v) => set({ armorKacBonusEq: v ?? 0 })} /></LabelInput>
          ...
          <LabelInput label="Pen. Armadura"><InputNumber style={{ width:'100%' }} value={data.armorAcPenalty ?? 0} onChange={(v) => set({ armorAcPenalty: v ?? 0 })} /></LabelInput>
          <LabelInput label="Adj. Velocidade"><Input value={data.armorSpeedAdj ?? ''} onChange={(e) => set({ armorSpeedAdj: e.target.value })} /></LabelInput>
          <LabelInput label="Slots Upgrade"><InputNumber style={{ width:'100%' }} value={data.armorUpgradeSlots} onChange={(v) => set({ armorUpgradeSlots: v ?? undefined })} /></LabelInput>
        </div>
        <div style={{ marginTop: 8 }}>
          <LabelInput label="Notas da Armadura"><Input.TextArea value={data.armorNotes ?? ''} onChange={(e) => set({ armorNotes: e.target.value })} rows={2} /></LabelInput>
        </div>
      </Collapse.Panel>

      {/* Abilities */}
      <Collapse.Panel header={`Habilidades de Classe (${abilities.length})`} key="abilities">
        <div style={{ display: 'grid', gap: 6 }}>
          ...
          <Button size="small" icon={<PlusOutlined />} onClick={() => set({ abilities: [...abilities, ''] })}>Adicionar habilidade</Button>
        </div>
      </Collapse.Panel>

      {/* Feats */}
      <Collapse.Panel header={`Talentos (${feats.length})`} key="feats">
        <div style={{ display: 'grid', gap: 6 }}>
          {feats.map((f, i) => (
            <Input.Group compact key={i} style={{ width:'100%', display:'flex' }}>
          ...
          <Button size="small" icon={<PlusOutlined />} onClick={() => set({ feats: [...feats, ''] })}>Adicionar talento</Button>
        </div>
      </Collapse.Panel>

      {/* Spells */}
      <Collapse.Panel header="Magias" key="spells">
        <LabelInput label="Total de magias conhecidas">
          ...
          {['1st','2nd','3rd','4th','5th','6th'].map((lvl) => {
            ...
            return (
              <div key={lvl} style={{ display: 'grid', gridTemplateColumns: 'auto repeat(3,1fr)', gap: 6, alignItems: 'center' }}>
                ...
                <LabelInput label="Conhecidas"><InputNumber size="small" style={{ width:'100%' }} value={s.spellsKnown} onChange={(v) => set({ spellSlots: { ...spellSlots, [lvl]: { ...s, spellsKnown: v ?? undefined } } })} /></LabelInput>
                <LabelInput label="Por dia"><InputNumber size="small" style={{ width:'100%' }} value={s.spellsPerDay} onChange={(v) => set({ spellSlots: { ...spellSlots, [lvl]: { ...s, spellsPerDay: v ?? undefined } } })} /></LabelInput>
                <LabelInput label="Usados"><InputNumber size="small" style={{ width:'100%' }} value={s.spellSlotsUsed} onChange={(v) => set({ spellSlots: { ...spellSlots, [lvl]: { ...s, spellSlotsUsed: v ?? undefined } } })} /></LabelInput>
              </div>
            );
          })}
        </div>
        <Divider style={{ margin: '10px 0' }} />
        <Typography.Text type="secondary" style={{ fontSize: 12 }}>Lista de magias conhecidas</Typography.Text>
        <div style={{ display: 'grid', gap: 6, marginTop: 8 }}>
          ...
          <Button size="small" icon={<PlusOutlined />} onClick={() => set({ spellsList: [...spellsList, ''] })}>Adicionar magia</Button>
        </div>
      </Collapse.Panel>

      {/* Equipment */}
      <Collapse.Panel header={`Equipamento (${equipment.length})`} key="equipment">
        <div style={{ display: 'grid', gap: 6 }}>
          {equipment.map((eq, i) => (
            <Space wrap key={i} size={4}>
              <Input placeholder="Nome" value={eq.name ?? ''} onChange={(e) => { const n=[...equipment]; n[i]={...n[i],name:e.target.value}; set({ equipment: n }); }} style={{ width: 200 }} />
              <Input placeholder="Nível" value={eq.level ?? ''} onChange={(e) => { const n=[...equipment]; n[i]={...n[i],level:e.target.value}; set({ equipment: n }); }} style={{ width: 70 }} />
          ...
          <Button size="small" icon={<PlusOutlined />} onClick={() => set({ equipment: [...equipment, {}] })}>Adicionar item</Button>
        </div>
        <Divider style={{ margin: '10px 0' }} />
        <Space wrap size={12}>
          <LabelInput label="Créditos"><InputNumber value={data.credits} onChange={(v) => set({ credits: v ?? undefined })} /></LabelInput>
          <LabelInput label="Outras riquezas"><Input value={data.otherWealth ?? ''} onChange={(e) => set({ otherWealth: e.target.value })} style={{ width: 200 }} /></LabelInput>
          <div>
            <Typography.Text type="secondary" style={{ fontSize: 11, display: 'block', marginBottom: 4 }}>Mochilas</Typography.Text>
            <Space>
              <Switch size="small" checked={!!data.backpacksCommercial} onChange={(v) => set({ backpacksCommercial: v })} /> <span style={{ fontSize: 12 }}>Comercial</span>
        ...
        <Space wrap size={12}>
          <CalcBox label="Sem encumbr. (Bulk)" value={calcStarfinder(data).carryUnenc} />
          <CalcBox label="Encumbrado (Bulk)" value={calcStarfinder(data).carryEnc} />
          <CalcBox label="Sobrecarregado (Bulk)" value={calcStarfinder(data).carryOver} />
        </Space>
      </Collapse.Panel>

      {/* Languages */}
      <Collapse.Panel header={`Idiomas (${languages.length})`} key="languages">
        <div style={{ display: 'grid', gap: 6 }}>
          {languages.map((l, i) => (
            <Input.Group compact key={i} style={{ width:'100%', display:'flex' }}>
          ...
          <Button size="small" icon={<PlusOutlined />} onClick={() => set({ languages: [...languages, ''] })}>Adicionar idioma</Button>
        </div>
      </Collapse.Panel>

      {/* XP */}
      <Collapse.Panel header="Experiência" key="xp">
        <Space size={16}>
          <LabelInput label="XP Ganho"><InputNumber value={data.xpEarned} onChange={(v) => set({ xpEarned: v ?? undefined })} /></LabelInput>
          <LabelInput label="Próximo Nível"><InputNumber value={data.xpNextLevel} onChange={(v) => set({ xpNextLevel: v ?? undefined })} /></LabelInput>
        </Space>
      </Collapse.Panel>
