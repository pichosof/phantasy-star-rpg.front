...
export interface GurpsRangedWeapon {
  weapon?: string;
  damage?: string;
  acc?: string;
  range?: string;
...
export interface GurpsPossession {
  item?: string;
  location?: string;
  cost?: number;
  weight?: number;
}
export interface GurpsListItem {
  name?: string;
  points?: number;
}
...
export interface SfEquipmentItem {
  name?: string;
  level?: string;
  bulk?: string;
}
export interface SfSpellLevel {
  spellsKnown?: number;
  spellsPerDay?: number;
...
export interface StarfinderSheetData {
  ...
  armorKacBonusEq?: number;
  armorEquipMaxDex?: number;
  armorBulk?: string;
  armorAcPenalty?: number;
  armorSpeedAdj?: string;
  armorUpgradeSlots?: number;
  armorNotes?: string;
  // Abilities / feats
  abilities?: string[];
  feats?: string[];
  ...
  // Equipment
  equipment?: SfEquipmentItem[];
  credits?: number;
  otherWealth?: string;
  backpacksCommercial?: boolean;
  backpacksIndustrial?: boolean;
  // Languages / XP
  languages?: string[];
  xpEarned?: number;
  xpNextLevel?: number;
...
export async function createCharacterSheet(body: {
  type: SheetType;
  name: string;
  data?: Record<string, unknown>;
